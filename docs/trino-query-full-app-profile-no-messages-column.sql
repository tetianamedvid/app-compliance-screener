-- =============================================================================
-- Full App Profile WITHOUT reading base44_app_agents_conversations_mongo.messages
-- =============================================================================
-- Use this when the full profile query fails with:
--   "Parquet page size ... exceeds maximum allowed size 1073741824 bytes for column [messages]"
-- The messages column in that table has an oversized Parquet page; until the table is compacted
-- or the cluster limit is raised, we cannot read it. This variant omits first_agent_conversation_*
-- from the agent_conversations table and sets those columns to NULL. All other columns are full.
-- When the data team fixes the table, switch back to trino-query-full-app-profile.sql.
-- =============================================================================
...
user_apps_logs_per_app AS (
  SELECT app_id, COUNT(*) AS user_app_events_count, MIN(created_date) AS first_activity_at, MAX(created_date) AS last_activity_at
  FROM prod.base44.base44_user_apps_logs_mongo
  WHERE app_id IS NOT NULL
  GROUP BY app_id
),
usage_per_app AS (
  SELECT app_id, COUNT(*) AS usage_record_count, MAX(created_date) AS last_usage_at
...
-- Do NOT read base44_app_agents_conversations_mongo.messages (Parquet page too large).
-- We still join for counts and use earliest_conversation_* for preview. first_agent_* = NULL.
...
user_counts_per_owner AS (
  SELECT m.wix_parent_account_id AS wix_account_id, COUNT(DISTINCT m.base44_user_id) AS base44_user_count
...
SELECT
  ...
  CAST(NULL AS varchar)                        AS first_agent_conversation_id,
  CAST(NULL AS timestamp)                      AS first_agent_conversation_created_date,
  CAST(NULL AS varchar)                        AS first_agent_conversation_agent_name,
  CAST(NULL AS varchar)                        AS first_agent_conversation_messages,
